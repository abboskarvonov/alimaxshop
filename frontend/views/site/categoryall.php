...
$slider = Slidercategory::find()->where(['menu_id' => $menu->id])->all();
$this->title = $title;
?>
            ...
            <div class="row">
                <ul class="nav">
                    ...
                    foreach ($menutitle as $menutitle) {
                        if($lang == 'uz'){
                            $menutit = $menutitle->name_uz; 
                        }
                        if($lang == 'ru'){
                            $menutit = $menutitle->name_ru; 
                        }
                    ?>
                    <li class="nav-item">
                        <a href="<?=\yii\helpers\Url::to(['site/category', 'link' => $menutitle->link])?>" class="nav-link" style="padding-top: 20px; padding-bottom: 20px; color: #000; font-weight: 600"><?=$menutit?></a>
                    </li>
                    <?
                    }
                    ?>
